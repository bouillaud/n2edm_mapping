...
run = int(sys.argv[1])
cycstart = int(sys.argv[2])
cycstep = int(sys.argv[3])
lmax = int(sys.argv[4])
if len(sys.argv)>5:
    dtformat = str(sys.argv[5])
    print(dtformat)
else:
    dtformat='v4_PhiZswap'

runmap = Run(run, np.arange(cycstart, 150, cycstep), dtformat=dtformat, calib=True, trim=True)
...
## save Glms
Gname = "spectrum_run{}_cycstart{}_lmax{}".format(run, cycstart, lmax)
Gerrname = "spectrumErr_run{}_cycstart{}_lmax{}".format(run, cycstart, lmax)
#i=0
...
plt.savefig("plots/spectrumPlot_run{}_cycstart{}_lmax{}".format(run, cycstart, lmax), dpi=200)
